...
enum BackgroundAssetStore {
    ...
    static func importImage(from source: URL, preferences: AppPreferences = .shared) throws -> URL {
        ...
        guard (values.fileSize ?? 0) <= 20 * 1_024 * 1_024 else {
            throw NSError(domain: "QuotaGlow.Background", code: 1, userInfo: [NSLocalizedDescriptionKey: "背景图片不能超过 20 MB"])
        }
        guard let image = NSImage(contentsOf: source), image.isValid else {
            throw NSError(domain: "QuotaGlow.Background", code: 2, userInfo: [NSLocalizedDescriptionKey: "无法读取这张图片"])
        }
        ...
        guard pixelWidth <= 8_192, pixelHeight <= 8_192 else {
            throw NSError(domain: "QuotaGlow.Background", code: 3, userInfo: [NSLocalizedDescriptionKey: "背景图片尺寸不能超过 8192 × 8192"])
        }
    ...
    private static func backgroundsDirectory() throws -> URL {
        let directory = try FileManager.default.url(
            ...
            appropriateFor: nil,
            create: true
        ).appendingPathComponent("QuotaGlow/Backgrounds", isDirectory: true)
